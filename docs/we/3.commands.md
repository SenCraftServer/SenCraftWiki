...
本页面列出了 WorldEdit 和 FAWE 的所有常用命令。

:::tip
在游戏中输入 `//help [分类|命令]` 可以查看命令的详细帮助信息！
:::
...
在使用命令前，先了解一下命令参数的表示方式：

| 语法 | 说明 | 示例 |
|------|------|------|
| `<arg>` | **必需参数**，必须提供 | `//set <方块>` |
| `[arg]` | **可选参数**，可省略 | `//undo [次数]` |
| `<arg1\|arg2>` | **多选一**，选择其中一个 | `//sel <cuboid\|sphere>` |
| `<arg=value>` | **默认值**，不指定时使用默认值 | |
| `-a` | **命令标志**，用于开启特定功能 | `//paste -a` |
...
| `//sel [cuboid/extend/poly/ellipsoid/sphere/cyl/convex] [-d]` |                          | [**选择区域类型**](./we/selections#选区)。支持的类型有：`cuboid`、`extend`、`poly`、`ellipsoid`、`sphere`、`cyl`、`convex `|
