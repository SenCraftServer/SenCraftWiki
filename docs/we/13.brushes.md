...
- [`/mat \<pattern\>`](./we/patterns) - 模式决定放置什么
- [`/mask \<mask\>`](./we/mask1) - 目标蒙版决定是否更改方块
- [`/smask \<mask\>`](./we/mask1) - 源蒙版更改方块是否可以放置
- [`/targetmask \<mask\>`](./we/mask1) - 笔刷的目标方块。默认蒙版是 `!air`
- [`/transform \<transform\>`](./we/transforms) - 变换改变方块放置的位置
- `/range \<range\>` - 笔刷使用的距离
